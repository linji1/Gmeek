...
class GMEEK():
    ...
    def createPostHtml(self,issue):
        ...
        if '<code class="notranslate">Gmeek-imgbox' in post_body: post_body = re.sub(r'<code class="notranslate">Gmeek-imgbox(.*?)</code>', lambda match: html.unescape(match.group(1)).replace('<img src="', '<img data-fancybox="gallery" data-src="').replace('">', '" data-src="') + '"', post_body, flags=re.DOTALL)
